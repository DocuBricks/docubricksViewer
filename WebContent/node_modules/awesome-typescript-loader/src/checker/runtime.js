...
function createChecker(receive, send) {
    ...
    class FileDeps {
        ...
        getDeps(containingFile) {
            return this.files[containingFile] || [];
        }
        getAllDeps(containingFile, allDeps = {}, initial = true) {
    ...
    class Host {
        ...
        resolveTypeReferenceDirectives(typeDirectiveNames, containingFile) {
            const resolved = typeDirectiveNames.map(directive => compiler.resolveTypeReferenceDirective(directive, containingFile, compilerOptions, compiler.sys)
        ...
        resolveModuleNames(moduleNames, containingFile) {
            const resolved = moduleNames.map(module => compiler.resolveModuleName(module, containingFile, compilerOptions, compiler.sys).resolvedModule);
            resolved.forEach(res => {
                if (res && res.resolvedFileName) {
                    fileDeps.add(containingFile, res.resolvedFileName);
                }
    ...
    function processInit({ seq, payload }) {
    ...
    function updateFile(fileName, text, ifExist = false) {
    ...
    function processUpdate({ seq, payload }) {
        updateFile(payload.fileName, payload.text, payload.ifExist);
        replyOk(seq, null);
    }
    function processRemove({ seq, payload }) {
        removeFile(payload.fileName);
        replyOk(seq, null);
    }
    function processEmit({ seq, payload }) {
    ...
    function processFiles({ seq }) {
        replyOk(seq, {
            files: service.getProgram().getSourceFiles().map(f => f.fileName)
        });
    }
    function processDiagnostics({ seq }) {
