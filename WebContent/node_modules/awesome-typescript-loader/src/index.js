...
function compiler(loader, text) {
    ...
    const rootCompiler = instance_1.getRootCompiler(loader._compiler);
    const query = (loaderUtils.getOptions(loader) || {});
    const options = (loader.options && loader.options.ts) || {};
    const instanceName = query.instance || 'at-loader';
    const instance = instance_1.ensureInstance(loader, query, options, instanceName, rootCompiler);
...
function transform(webpack, instance, fileName, text) {
    ...
    return instance.checker.emitFile(fileName, text).then((({ emitResult, deps }) => {
        ...
        if (resultSourceMap) {
            let sourcePath = path.relative(instance.compilerConfig.options.sourceRoot || instance.context, loaderUtils.getRemainingRequest(webpack));
